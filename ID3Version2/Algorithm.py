...
def totalDepth(root,level=0):
    ...
    if root.left is None:
        if root.right is None:
            ...
            return totalDepth(root.right, level + 1)
    elif root.right is None:
        return totalDepth(root.left, level + 1)
    else:
...
def RandomAlgorithm(trainingValues,inputAttributes,binaryTreeNode,targetAttribute):
    plusCount = 0
    minusCount = 0
    totalRecords = 0
    inputSetToCheckCount = trainingValues.copy()
    ...
    if plusCount == totalRecords:
        ...
        resultOfBestAttribute = findRandomAttribute(trainingValues,inputAttributes)
        binaryTreeNode.testAttrForCurrentNode = resultOfBestAttribute.decisionAttribute
        binaryTreeNode.dataForPruning = trainingValues.copy()
...
def findRandomAttribute(inputSet,inputAttributes):
    attrIndex = random.randint(0, len(inputAttributes))
    currentAttribute = inputAttributes[attrIndex]
    currentSet = inputSet.copy()
    leftSet = {}
    rightSet = {}
    leftSetIndex = 0
...
def pruningAlgorithm(decisionTree,K,validation_filename):
   ...
   print("Accuracy DT against Test set: ")
   print(str(maxAccuracyWithID3))
   print("Average depth: ")
   average = (totalDepth(decisionTree, level=0)) / (DT_leaf_nodes(decisionTree))
   print(average)
